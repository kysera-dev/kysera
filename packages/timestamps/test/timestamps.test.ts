...
describe('Timestamps Plugin', () => {
  ...
  describe('Integration', () => {
    ...
    it('should handle multiple plugins together', async () => {
      ...
      const userRepo = await createTestRepository(db, 'users', [
        auditPlugin,
        timestampsPluginInstance
      ])
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle empty table for findCreatedBetween', async () => {
      ...
      const results = await userRepo.findCreatedBetween(
  ...
  describe('Date Format Options', () => {
    ...
    it('should handle touch with date format options', async () => {
      ...
      const touched = await userRepo.findById(user.id)
      const updatedAtNum =
        typeof touched.updated_at === 'string'
          ? parseInt(touched.updated_at as string, 10)
          : touched.updated_at
  ...
  describe('Plugin Metadata', () => {
    ...
    it('should not have interceptQuery method', () => {
      const plugin = timestampsPlugin()
      // The timestamps plugin does not need to intercept queries.
      // It only extends repository methods to add timestamps on create/update operations.
      // This is different from soft-delete plugin which needs to filter SELECT queries.
      expect(plugin.interceptQuery).toBeUndefined()
    })

    it('should have extendRepository method', () => {
      const plugin = timestampsPlugin()
      // The timestamps plugin uses extendRepository to override create/update methods
      expect(plugin.extendRepository).toBeDefined()
      expect(typeof plugin.extendRepository).toBe('function')
    })
  })
})
