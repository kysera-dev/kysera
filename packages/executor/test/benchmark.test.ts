...
describe('@kysera/executor - Performance Benchmarks', () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 1; i <= 100; i++) {
      sqlite.exec(
        `INSERT INTO users (id, name, tenant_id, deleted_at) VALUES (${i}, 'User${i}', ${i % 10}, NULL)`
      )
    }
  })
  ...
  const runBenchmark = async (
    ...
    console.log(
      `${label}: ${qps.toFixed(0)} queries/sec (${duration.toFixed(2)}ms for ${iterations} queries)`
    )
  ...
  it('should have minimal overhead without plugins (zero overhead path)', async () => {
    ...
    const baseline = await runBenchmark('Pure Kysely', () =>
      db.selectFrom('users').selectAll().execute()
    )

    const withExecutor = await runBenchmark('Executor (no plugins)', () =>
  ...
  it('should have acceptable overhead with 1 interceptor plugin', async () => {
    ...
    const withPlugin = await runBenchmark('Executor (1 plugin)', () =>
      executor.selectFrom('users').selectAll().execute()
    )
  ...
  it('should have acceptable overhead with 3 interceptor plugins', async () => {
    ...
    const withPlugins = await runBenchmark('Executor (3 plugins)', () =>
      executor.selectFrom('users').selectAll().execute()
    )
  ...
  it('should have acceptable overhead with 5 interceptor plugins', async () => {
    ...
    const withPlugins = await runBenchmark('Executor (5 plugins)', () =>
      executor.selectFrom('users').selectAll().execute()
    )
  ...
  it('should have no overhead for plugins without interceptQuery', async () => {
    ...
    const withPlugins = await runBenchmark('Executor (no interceptors)', () =>
      executor.selectFrom('users').selectAll().execute()
    )
  ...
  it('should benchmark sync executor creation', () => {
    ...
    console.log(
      `Sync executor creation: ${((iterations / syncDuration) * 1000).toFixed(0)} creations/sec`
    )
  ...
  it('should efficiently handle different query types', async () => {
    ...
    const insertQps = await runBenchmark(
      ...
      () =>
        executor
          .insertInto('users')
          .values({ id: ++insertCounter, name: 'Test', tenant_id: 1, deleted_at: null })
          .execute(),
      500
    )
