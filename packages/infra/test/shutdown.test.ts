...
import { gracefulShutdown, shutdownDatabase, createShutdownController } from '../src/shutdown.js'
...
describe('gracefulShutdown', () => {
  ...
  it('should timeout if shutdown takes too long', async () => {
    ...
    await expect(gracefulShutdown(db as any, { timeout: 50 })).rejects.toThrow(
  ...
  it('should clear timeout timer after shutdown completes', async () => {
    const db = createMockDb()

    // Use fake timers to track timer cleanup
    vi.useFakeTimers()

    const clearTimeoutSpy = vi.spyOn(globalThis, 'clearTimeout')

    // Create a fast-resolving destroy
    db.destroy.mockResolvedValue(undefined)

    const shutdownPromise = gracefulShutdown(db as any, { timeout: 1000 })
    ...
    // The finally block in gracefulShutdown ensures this happens
