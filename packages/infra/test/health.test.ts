 ...
 */
function createMockDbWithMetrics<DB>(
  metrics: QueryMetrics[]
): Kysely<DB> & DatabaseWithMetrics<DB> {
...
describe('HealthMonitor', () => {
  ...
  it('should clean up resources with Symbol.dispose()', async () => {
    const db = createMockDb({})
    const monitor = new HealthMonitor(db)
  ...
  it('should clean up resources with destroy() method', async () => {
    const db = createMockDb({})
    const monitor = new HealthMonitor(db)
  ...
  it('should not leak timers when disposed without explicit stop', async () => {
    const db = createMockDb({})
    const onCheck = vi.fn()

    // Create and start monitor in a scope
    {
      const monitor = new HealthMonitor(db, { intervalMs: 1000 })
      monitor.start(onCheck)
      ...
      // Dispose without calling stop()
      monitor[Symbol.dispose]()
    }

    // Advance time significantly - no more checks should happen
    onCheck.mockClear()
