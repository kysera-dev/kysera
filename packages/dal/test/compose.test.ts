...
describe('compose', () => {
  ...
  it('should create context from raw Kysely instance (covers toContext line 19)', async () => {
    ...
    const first: QueryFunction<TestDB, [number], { id: number }> = vi.fn().mockImplementation((ctx) => {
      capturedCtx = ctx;
...
describe('parallel', () => {
  ...
  it('should pass same args to all queries', async () => {
    ...
    const combined = parallel<TestDB, [string, string], { q1: typeof q1; q2: typeof q2 }>({ q1, q2 });
