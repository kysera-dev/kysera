...
import { TransactionRequiredError } from '../src/errors.js'
...
describe('createQuery', () => {
  ...
  describe('type inference', () => {
    ...
    it('should correctly handle undefined return type', async () => {
      const query = createQuery<TestDB, [number], undefined>((_ctx, _id: number) =>
...
describe('createTransactionalQuery', () => {
  describe('when called within transaction', () => {
    ...
    it('should pass arguments correctly within transaction', async () => {
      ...
      const query = createTransactionalQuery<
        TestDB,
  ...
  describe('when called outside transaction', () => {
    it('should throw TransactionRequiredError when not in transaction with context', async () => {
      const queryFn = vi.fn().mockResolvedValue({ success: true })
      const query = createTransactionalQuery<TestDB, [], { success: boolean }>(queryFn)

      const ctx = createMockContext(false) // isTransaction = false

      await expect(query(ctx)).rejects.toThrow(TransactionRequiredError)
    ...
    it('should throw TransactionRequiredError when called with Kysely directly', async () => {
      const queryFn = vi.fn().mockResolvedValue({ success: true })
      ...
      await expect(query(db, 1)).rejects.toThrow(TransactionRequiredError)
      expect(queryFn).not.toHaveBeenCalled()
    })
    ...
    it('should throw error with correct name', async () => {
      ...
      try {
        ...
        expect((error as Error).name).toBe('TransactionRequiredError')
      }
    })
  })
