...
import { checkDatabaseHealth, HealthMonitor, gracefulShutdown, registerShutdownHandlers, createMetricsPool } from '../src/health'
import { createTestDatabase, initializeTestSchema } from './setup/test-database'
...
describe('Health Checks with Real SQLite Database', () => {
  ...
  describe('gracefulShutdown', () => {
    ...
    it('should timeout if shutdown takes too long', async () => {
      ...
      await expect(
        gracefulShutdown(testDb, { timeoutMs: 100 })
      ).rejects.toThrow('Shutdown timeout after 100ms')
    })
    ...
    it('should handle shutdown errors', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Error during database shutdown'), expect.any(Error))
