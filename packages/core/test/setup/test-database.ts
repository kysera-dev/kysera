...
export async function initializeTestSchema(db: Kysely<TestDatabase>): Promise<void> {
  ...
  await db.schema.createIndex('idx_posts_deleted_at').on('posts').column('deleted_at').execute()
}
...
export const testFactories = {
  user(
    overrides?: Partial<Omit<UsersTable, 'id' | 'created_at'>>
  ): Omit<UsersTable, 'id' | 'created_at'> {
    return {
  ...
  post(
    user_id: number,
    overrides?: Partial<Omit<PostsTable, 'id' | 'created_at'>>
  ): Omit<PostsTable, 'id' | 'created_at'> {
    return {
      user_id,
      title: 'Test Post',
  ...
  comment(
    post_id: number,
    user_id: number,
    overrides?: Partial<Omit<CommentsTable, 'id' | 'created_at'>>
  ): Omit<CommentsTable, 'id' | 'created_at'> {
    return {
      post_id,
      user_id,
