...
import { paginate, paginateCursor, paginateCursorSimple } from '../src/pagination'
import { createTestDatabase, initializeTestSchema, clearTestDatabase, testFactories } from './setup/test-database'
  ...
  describe('Offset-based Pagination', () => {
    ...
    it('should handle complex queries with joins', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        await db
          .insertInto('posts')
          .values(testFactories.post(i % 2 === 0 ? user1.id : user2.id, {
            title: `Post ${i + 1}`
          }))
          .execute()
      }
