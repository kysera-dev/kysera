...
describe('Query Helpers', () => {
  ...
  describe('applyOffset', () => {
    ...
    it('should support infinite scroll pattern', async () => {
      ...
      const page1 = await applyOffset(db.selectFrom('posts').selectAll().orderBy('id'), {
        limit: 2,
        offset: 0
      }).execute()

      const page2 = await applyOffset(db.selectFrom('posts').selectAll().orderBy('id'), {
        limit: 2,
        offset: 2
      }).execute()
    ...
    it('should preserve existing WHERE clauses', async () => {
      const query = db.selectFrom('posts').selectAll().where('published', '=', 1).orderBy('id')
  ...
  describe('Combining applyOffset and applyDateRange', () => {
    it('should work together for paginated date-filtered results', async () => {
      ...
      const result = await applyOffset(
        applyDateRange(db.selectFrom('posts').selectAll().orderBy('created_at'), 'created_at', {
          from: posts[0]!.created_at,
          to: posts[2]!.created_at
        }),
        { limit: 2, offset: 0 }
    ...
    it('should preserve order when combining helpers', async () => {
      ...
      const result = await applyOffset(
        applyDateRange(db.selectFrom('posts').selectAll().orderBy('id', 'asc'), 'created_at', {
          from: posts[0]!.created_at
        }),
        { limit: 2 }
      ).execute()
  ...
  describe('Edge Cases', () => {
    it('should handle applyOffset with limit 1 correctly', async () => {
      const result = await applyOffset(db.selectFrom('users').selectAll().orderBy('id'), {
        limit: 1
      }).execute()
    ...
    it('should work with selectAll and specific columns', async () => {
      const result1 = await applyOffset(db.selectFrom('posts').selectAll(), { limit: 1 }).execute()

      const result2 = await applyOffset(db.selectFrom('posts').select(['id', 'title']), {
        limit: 1
      }).execute()
