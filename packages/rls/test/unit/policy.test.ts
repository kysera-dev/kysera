...
describe('defineRLSSchema', () => {
  ...
  it('should throw on invalid policy type', () => {
    expect(() =>
      defineRLSSchema({
        users: {
          policies: [{ type: 'invalid' as any, operation: 'read', condition: () => true }]
        }
      })
  ...
  it('should throw on missing operation', () => {
    expect(() =>
      defineRLSSchema({
        users: {
          policies: [{ type: 'allow', operation: undefined as any, condition: () => true }]
        }
      })
    ).toThrow(RLSSchemaError)
  })
  ...
  it('should throw on missing condition', () => {
    expect(() =>
      defineRLSSchema({
        users: {
          policies: [{ type: 'allow', operation: 'read', condition: undefined as any }]
        }
      })
    ).toThrow(RLSSchemaError)
  })
  ...
  it('should throw on invalid condition type', () => {
    expect(() =>
      defineRLSSchema({
        users: {
          policies: [{ type: 'allow', operation: 'read', condition: 123 as any }]
        }
      })
    ).toThrow(RLSSchemaError)
  })
