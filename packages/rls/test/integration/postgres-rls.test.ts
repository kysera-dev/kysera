/**
 * PostgreSQL Integration Tests for @kysera/rls
 *
 * Tests RLS policy enforcement with real PostgreSQL database.
...
import { PostgresRLSGenerator } from '../../src/native/postgres.js';
import { RLSMigrationGenerator } from '../../src/native/migration.js';

// Skip tests if PostgreSQL is not available
const isPostgresAvailable =
  process.env['TEST_POSTGRES'] === 'true' || process.env['CI'] === 'true';

describe.skipIf(!isPostgresAvailable)('PostgreSQL Integration Tests', () => {
  ...
  describe('PostgreSQL-Specific Features', () => {
    it('should handle PostgreSQL JSONB operations', async (ctx) => {
      ...
      // Test that RLS works with PostgreSQL-specific types
  ...
  describe('Native PostgreSQL RLS Generation', () => {
    ...
    it('should generate valid PostgreSQL RLS statements', () => {
      const generator = new PostgresRLSGenerator();
      const statements = generator.generateStatements(nativeSchema);

      // Should have statements for users and posts tables
    ...
    it('should generate migration with context functions', () => {
      const migrationGenerator = new RLSMigrationGenerator();
      const migration = migrationGenerator.generateMigration(nativeSchema);

      // Should contain context function definitions
      expect(migration).toContain('rls_current_user_id');
      expect(migration).toContain('rls_current_tenant_id');
      expect(migration).toContain('STABLE');
    });

    it('should generate DROP statements for rollback', () => {
      const generator = new PostgresRLSGenerator();
      const dropStatements = generator.generateDropStatements(nativeSchema);

      // Should have DROP POLICY statements
      expect(dropStatements.length).toBeGreaterThan(0);
      expect(dropStatements.some((s) => s.includes('DROP POLICY'))).toBe(true);
    });
  });
