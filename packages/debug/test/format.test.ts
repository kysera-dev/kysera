...
describe('formatSQLPretty', () => {
  ...
  it('should handle nested parentheses', () => {
    const sql = 'SELECT * FROM users WHERE id IN (SELECT user_id FROM orders WHERE status IN (1, 2))';
    const formatted = formatSQLPretty(sql);
  ...
  it('should handle deeply nested subqueries', () => {
    const sql = 'SELECT * FROM a WHERE x IN (SELECT * FROM b WHERE y IN (SELECT * FROM c WHERE z IN (1)))';
    const formatted = formatSQLPretty(sql);
