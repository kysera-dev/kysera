    ...
    const mockValibotModule: any = {
      ...
      safeParse: (_schema: any, data: unknown): any => {
        try {
          ...
          return {
            success: false,
            issues: [{ type: 'custom', message: err instanceof Error ? err.message : 'Validation failed' }],
          };
        }
      },
    };
...
describe('Integration with Repository patterns', () => {
  describe('Entity validation', () => {
    ...
    it('should work with nativeAdapter for no-validation scenarios', () => {
      ...
      const entity = entityValidator.parse({ id: 1, name: 'John', email: 'john@example.com', createdAt: new Date() });
...
describe('Error handling edge cases', () => {
  ...
  it('should handle union type validation', () => {
    const schema = z.union([z.object({ type: z.literal('a'), value: z.string() }), z.object({ type: z.literal('b'), value: z.number() })]);
