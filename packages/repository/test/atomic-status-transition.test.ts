...
// eslint-disable-next-line max-lines-per-function -- Comprehensive test suite for atomic status transitions
describe('atomicStatusTransition', () => {
  ...
  describe('Basic Status Transition', () => {
    ...
    it('should work without returning (returning: false)', async () => {
      ...
      // When returning: false, function returns void
  ...
  describe('Real-world Scenarios', () => {
    it('should prevent double-spend in payment processing scenario', async () => {
      ...
      const [worker1Result, worker2Result] = await Promise.all([worker1Promise, worker2Promise])
    ...
    it('should handle cancellation from multiple valid states', async () => {
      ...
      const placedOrder = await db
        .insertInto('orders')
        .values({ customer_id: 1, order_status: OrderStatus.Placed, total: 50.0 })
        .returningAll()
        .executeTakeFirstOrThrow()

      const confirmedOrder = await db
        .insertInto('orders')
        .values({ customer_id: 2, order_status: OrderStatus.Confirmed, total: 75.0 })
        .returningAll()
        .executeTakeFirstOrThrow()

      const shippedOrder = await db
        .insertInto('orders')
        .values({ customer_id: 3, order_status: OrderStatus.Shipped, total: 100.0 })
        .returningAll()
        .executeTakeFirstOrThrow()
