...
describe('Keyset-based Cursor Pagination', () => {
  ...
  describe('Performance Characteristics', () => {
    it('should use keyset pagination (no OFFSET in query)', async () => {
      ...
      for (let i = 1; i <= 100; i++) {
        createPromises.push(userRepo.create({ email: `user${i}@example.com`, name: `User ${i}` }))
      }
      ...
      for (let i = 0; i < 9; i++) {
        const page: Awaited<ReturnType<typeof userRepo.paginateCursor>> =
          await userRepo.paginateCursor({
            limit: 10,
            cursor
          })
