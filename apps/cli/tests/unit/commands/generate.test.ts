...
describe('generate model command', () => {
  ...
  describe('error handling', () => {
    it('should throw error if config not found', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', 'users'])).rejects.toThrow()
    })
...
describe('generate crud command', () => {
  ...
  describe('crud generation', () => {
    it('should generate model, repository, and schema', async () => {
      ...
      expect(
        writeCalls.some(c => c[0].includes('repositories') && c[0].includes('.repository.ts'))
      ).toBe(true)

      // Should generate schema
      expect(writeCalls.some(c => c[0].includes('schemas') && c[0].includes('.schema.ts'))).toBe(
        true
      )
    })
  ...
  describe('error handling', () => {
    it('should throw error for nonexistent table', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', 'nonexistent'])).rejects.toThrow()
    })
