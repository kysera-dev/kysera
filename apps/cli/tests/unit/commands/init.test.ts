...
describe('init command', () => {
  ...
  describe('project generation', () => {
    ...
    it('should create additional directories for api template', async () => {
      ...
      expect(
        mkdirCalls.some((p: string) => p.includes('controllers') || p.includes('routes'))
      ).toBe(true)
    })
    ...
    it('should create graphql directories for graphql template', async () => {
      ...
      expect(mkdirCalls.some((p: string) => p.includes('resolvers') || p.includes('schema'))).toBe(
        true
      )
    })
    ...
    it('should create monorepo structure for monorepo template', async () => {
      ...
      expect(mkdirCalls.some((p: string) => p.includes('apps') || p.includes('packages'))).toBe(
        true
      )
    })
  })
  ...
  describe('directory conflict handling', () => {
    it('should throw error if directory exists and is not empty', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', 'existing-project', '--no-git', '--no-install'])
      ).rejects.toThrow()
    })
    ...
    it('should succeed if directory exists but is empty', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', 'empty-project', '--no-git', '--no-install'])
    ...
    it('should allow using current directory with "."', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', '.', '--no-git', '--no-install'])
      ).resolves.not.toThrow()
    })
  })
  ...
  describe('git initialization', () => {
    ...
    it('should handle git init failure gracefully', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', 'git-fail-project', '--git', '--no-install'])
      ).resolves.not.toThrow()
    })
  })
  ...
  describe('dependency installation', () => {
    it('should install dependencies when --install is true', async () => {
      ...
      expect(execa).toHaveBeenCalledWith(expect.any(String), ['install'], expect.any(Object))
    })
