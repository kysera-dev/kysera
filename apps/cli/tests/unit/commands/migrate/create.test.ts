...
describe('migrate create command', () => {
  ...
  describe('error handling', () => {
    it('should throw error for invalid template', async () => {
      await expect(
        command.parseAsync(['node', 'test', 'test_migration', '--template', 'invalid'])
    ...
    it('should throw error if migration file already exists', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', 'test_migration'])).rejects.toThrow(CLIError)
    })

    it('should throw error when table template requires --table option', async () => {
      await expect(
        command.parseAsync(['node', 'test', 'alter_table', '--template', 'alter-table'])
      ).rejects.toThrow(CLIError)
    })
  })
