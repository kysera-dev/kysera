...
describe('query analyze command', () => {
  ...
  describe('success scenarios', () => {
    it('should analyze a query successfully', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', '--query', 'SELECT * FROM users'])
  ...
  describe('error handling', () => {
    ...
    it('should throw error when database config is not found', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', '--query', 'SELECT 1'])).rejects.toThrow(
        CLIError
      )
    })
    ...
    it('should throw error when query file cannot be read', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', '--file', './nonexistent.sql'])
    ...
    it('should handle invalid benchmark value', async () => {
      ...
      await expect(
        command.parseAsync(['node', 'test', '--query', 'SELECT 1', '--benchmark', 'abc'])
      ).resolves.not.toThrow()
