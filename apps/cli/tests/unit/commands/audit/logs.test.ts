...
describe('audit logs command', () => {
  ...
  describe('error handling', () => {
    ...
    it('should handle invalid since date', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', '--since', 'invalid-date'])).rejects.toThrow(
        CLIError
      )
    })
    ...
    it('should handle invalid until date', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', '--until', 'invalid-date'])).rejects.toThrow(
        CLIError
      )
    })
    ...
    it('should handle invalid limit value', async () => {
      ...
      await expect(command.parseAsync(['node', 'test', '--limit', 'abc'])).rejects.toThrow(CLIError)
    })
  })
