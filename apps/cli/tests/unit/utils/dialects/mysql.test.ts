...
describe('MySQL Dialect Utilities', () => {
  ...
  describe('createDatabase', () => {
    ...
    it('should create database with custom charset and collation', async () => {
      ...
      expect(sql.raw).toHaveBeenCalledWith(
        'CREATE DATABASE `newdb` CHARACTER SET latin1 COLLATE latin1_swedish_ci'
      )
    })
    ...
    it('should throw CLIDatabaseError on failure', async () => {
      ...
      await expect(createDatabase(mockDb, 'existingdb')).rejects.toThrow(
    ...
    it('should reject invalid charset', async () => {
      ...
      await expect(
        createDatabase(mockDb, 'newdb', "utf8'; DROP TABLE", 'utf8_general_ci')
      ).rejects.toThrow()
    })
    ...
    it('should reject invalid collation', async () => {
      ...
      await expect(
        createDatabase(mockDb, 'newdb', 'utf8', "utf8_general_ci'; DROP")
      ).rejects.toThrow()
    })
  })
