...
describe('SQLite Dialect Utilities', () => {
  ...
  describe('getIndexInfo', () => {
    ...
    it('should throw CLIDatabaseError on failure', async () => {
      ...
      await expect(getIndexInfo(mockDb, 'nonexistent_idx')).rejects.toThrow(
  ...
  describe('getForeignKeys', () => {
    ...
    it('should throw CLIDatabaseError on failure', async () => {
      ...
      await expect(getForeignKeys(mockDb, 'nonexistent')).rejects.toThrow(
        'Failed to get foreign keys'
      )
    })
  })
  ...
  describe('getTableStatistics', () => {
    ...
    it('should throw CLIDatabaseError on failure', async () => {
      ...
      await expect(getTableStatistics(mockDb, 'nonexistent')).rejects.toThrow(
  ...
  describe('createBackup', () => {
    ...
    it('should throw CLIDatabaseError on failure', async () => {
      ...
      await expect(createBackup(mockDb, '/path/to/backup.db')).rejects.toThrow(
          ...
          select: vi.fn().mockReturnValue({
            where: vi.fn().mockReturnValue({
              where: vi.fn().mockReturnValue({
                execute: vi
                  .fn()
                  .mockResolvedValue([{ name: 'users' }, { name: 'posts' }, { name: 'comments' }])
              })
            })
          })
        })
          ...
          select: vi.fn().mockReturnValue({
            where: vi.fn().mockReturnValue({
              where: vi.fn().mockReturnValue({
                execute: vi
                  .fn()
                  .mockResolvedValue([{ name: 'idx_implicit', table: 'users', sql: null }])
              })
            })
          })
        })
