...
export function resolveConfigPaths(config: KyseraConfig, configPath: string): KyseraConfig {
  ...
  if (resolved.migrations?.templates?.create) {
    resolved.migrations.templates.create = resolvePath(
      resolved.migrations.templates.create,
      configPath
    )
  }
  if (resolved.migrations?.templates?.table) {
    resolved.migrations.templates.table = resolvePath(
      resolved.migrations.templates.table,
      configPath
    )
  }
  ...
  if (resolved.generate?.templates?.repository) {
    resolved.generate.templates.repository = resolvePath(
      resolved.generate.templates.repository,
      configPath
    )
  }
