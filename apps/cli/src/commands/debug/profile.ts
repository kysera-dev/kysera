...
function displayProfileResults(
  ...
  compareResult: ProfileResult | undefined,
  dialect: string
): void {
  console.log('')
  ...
  if (compareResult) {
    ...
    const improvement = (
      ((compareResult.avgDuration - result.avgDuration) / compareResult.avgDuration) *
      100
    ).toFixed(1)
  ...
  console.log(
    prism.gray(
      `Profiled with ${result.iterations} iterations after ${result.warmupRuns} warmup runs`
    )
  )
}
