...
function displayHealthResults(result: HealthCheckResult, verbose: boolean): void {
  ...
  // Overall status
  const statusIcon =
    result.status === 'healthy' ? '[OK]' : result.status === 'degraded' ? '[WARN]' : '[ERR]'
  const statusColor =
    result.status === 'healthy'
      ...
      : result.status === 'degraded'
        ...
        : prism.red
  console.log(
    `Status: ${statusColor(statusIcon + ' ' + result.status.charAt(0).toUpperCase() + result.status.slice(1))}`
  )
  console.log('')

  // Connection info
  console.log('Connection:')
  const connChecks = result.checks.filter(
    c => c.name.includes('connection') || c.name.includes('Connection')
  )
  ...
  if (verbose) {
    const otherChecks = result.checks.filter(
      c => !c.name.includes('connection') && !c.name.includes('Connection')
    )
