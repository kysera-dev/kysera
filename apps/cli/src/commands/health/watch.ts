    ...
    while (isRunning) {
      ...
      try {
        ...
        if (consecutiveFailures >= maxConsecutiveFailures) {
          console.log('')
          console.log(
            prism.red(`⚠️  ALERT: ${consecutiveFailures} consecutive health check failures!`)
          )
...
function displayMonitoringResult(result: any, verbose: boolean, consecutiveFailures: number): void {
  ...
  if (result.metrics?.poolMetrics) {
    ...
    console.log(
      `Connections: ${pool.activeConnections}/${pool.totalConnections} (${pool.idleConnections} idle)`
    )
  }
