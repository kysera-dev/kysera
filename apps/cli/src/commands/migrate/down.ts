...
async function rollbackMigrations(options: DownOptions): Promise<void> {
  ...
  try {
    ...
    if (rolledBack.length > 0) {
      ...
      if (options.dryRun) {
        logger.info(
          prism.yellow(`Would have rolled back ${rolledBack.length} migration${rolledBack.length > 1 ? 's' : ''} (${duration}ms)`)
        )
      } else {
        logger.info(
          prism.green(`✅ ${rolledBack.length} migration${rolledBack.length > 1 ? 's' : ''} rolled back successfully (${duration}ms)`)
        )
      }
    }
