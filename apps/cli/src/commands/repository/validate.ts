    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
          ...
          entry.isFile() &&
          (entry.name.endsWith('Repository.ts') || entry.name.endsWith('Repository.js'))
        ) {
          files.push(fullPath)
        }
      }
...
function displayValidationResults(
  ...
  if (validRepos.length > 0 && options.showDetails) {
    ...
    for (const repo of validRepos) {
      ...
      if (repo.issues.length > 0) {
        console.log(
          `    ${repo.issues.length} non-critical issue${repo.issues.length !== 1 ? 's' : ''}`
        )
      }
    }
  }
  ...
  if (fixableCount > 0 && !options.fix) {
    ...
    console.log(
      prism.yellow(
        `💡 ${fixableCount} issue${fixableCount !== 1 ? 's' : ''} can be automatically fixed. Use --fix to apply.`
      )
    )
  }
}
    ...
    if (fixableIssues.length > 0) {
      ...
      try {
        ...
        if (updatedContent !== content) {
          ...
          fixSpinner.succeed(
            `Fixed ${fixableIssues.length} issue${fixableIssues.length !== 1 ? 's' : ''} in ${result.repository}`
          )
        } else {
