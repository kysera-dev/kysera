...
async function listRepositories(options: ListRepositoriesOptions): Promise<void> {
  ...
  try {
    ...
    listSpinner.succeed(
      `Found ${repositoryFiles.length} repository file${repositoryFiles.length !== 1 ? 's' : ''}`
    )
...
async function parseRepositoryFile(
  filePath: string,
...
function displayRepositories(
  ...
  console.log(
    `  With Validation: ${withValidation} (${Math.round((withValidation / repositories.length) * 100)}%)`
  )
  console.log(
    `  With Pagination: ${withPagination} (${Math.round((withPagination / repositories.length) * 100)}%)`
  )
  console.log(
    `  With Soft Delete: ${withSoftDelete} (${Math.round((withSoftDelete / repositories.length) * 100)}%)`
  )
