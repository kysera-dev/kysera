...
function parseMethods(
  ...
  while ((match = methodRegex.exec(content)) !== null) {
    ...
    const queryMatches = methodBody.matchAll(
      /\.(selectFrom|insertInto|updateTable|deleteFrom)\(['"`](\w+)['"`]\)/g
    )
...
function calculateStats(
  methods: RepositoryInspection['methods'],
  totalLines: number
): RepositoryInspection['stats'] {
...
function displayInspection(
  ...
  if (inspection.schema) {
    console.log('')
    console.log(
      prism.cyan(`✅ Validation Schema: ${inspection.schema.name} (${inspection.schema.type})`)
    )
    console.log('  Properties:')
