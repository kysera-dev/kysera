...
export class ConfigCache {
   ...
   */
  static async load(path: string, loader: () => Promise<any>): Promise<any> {
...
export class ConnectionPool {
   ...
   */
  static async get(key: string, factory: () => Promise<Kysely<any>>): Promise<Kysely<any>> {
    ...
    if (conn) {
      ...
      if (conn.refs <= 0) {
        ...
        setTimeout(() => {
          ...
          if (current && current.refs <= 0 && Date.now() - current.lastAccess > this.idleTimeout) {
            this.close(key)
          }
        }, this.idleTimeout)
      }
  ...
  static async closeAll(): Promise<void> {
    ...
    const closePromises = Array.from(this.connections.entries()).map(async ([key, conn]) => {
      await conn.db.destroy()
    })
  ...
  private static async evictIdle(): Promise<void> {
    ...
    const idle = Array.from(this.connections.entries())
      .filter(([_, conn]) => conn.refs === 0 && now - conn.lastAccess > this.idleTimeout)
      .map(([key]) => key)
...
export class SchemaCache {
   ...
   */
  static async load(database: string, loader: () => Promise<any>): Promise<any> {
