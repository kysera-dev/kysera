...
- **No New Features** - This is purely a cleanup release to prepare for v1.0
...
### 1. RLS Plugin: `skipTables` Option Removed

The deprecated `skipTables` option in the RLS plugin has been completely removed. This option was deprecated in v0.7.0 in favor of the more descriptive `excludeTables` option.

**Impact:** If you're still using `skipTables`, your code will break in v0.8.

### 2. `DatabaseDialect` Type Alias Removed
...
### 1. RLS Plugin: `skipTables` Removed
...
This is the complete list of APIs removed in v0.8.0:

### @kysera/rls

| Removed API     | Replacement       | Package        |
| --------------- | ----------------- | -------------- |
| `skipTables`    | `excludeTables`   | `@kysera/rls`  |
...
- [ ] **Update tests** - All test files migrated
